...
func runCDCKafkaAuth(ctx context.Context, t *test, c *cluster) {
	...
	if err := db.QueryRow(
		`CREATE CHANGEFEED FOR auth_test_table INTO $1`,
		fmt.Sprintf("%s?tls_enabled=true&ca_cert=%s&sasl_enabled=true&sasl_user=plain&sasl_password=plain-secret&sasl_mechanism=PLAIN", kafka.sinkURLSASL(ctx), testCerts.CACertBase64()),
	).Scan(&jobID); err != nil {
